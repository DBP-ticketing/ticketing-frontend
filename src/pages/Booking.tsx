...
export default function Booking() {
  ...
  // 자유좌석/스탠딩석 예매 처리
  const handleFreeOrStandingBooking = async () => {
    setProcessing(true);
    try {
      // eventId만 전달 (좌석은 자동 배정)
  ...
  // 지정좌석 예매 처리
  const handleAssignedBooking = async () => {
    if (!selectedSeat) {
              ...
              <div className="card sticky top-24">
                ...
                {selectedSeat ? (
                  <>
                    ...
                    <button onClick={handleAssignedBooking} disabled={processing} className="btn-primary w-full">{processing ? '처리 중...' : '결제하기'}</button>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            {event.seatForm === 'FREE' ? '자유좌석 - 좌석은 선착순으로 자동 배정됩니다' : '스탠딩 - 입장권 구매 후 자유롭게 관람하실 수 있습니다'}
          </p>
        </div>
        ...
        <div className="card">
          <div className="text-center py-12">
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-4">예매 가능 좌석</h2>
            <p className="text-5xl font-bold text-primary mb-8">{availableSeatsCount}석</p>
                ...
                <div className="inline-block bg-gray-50 rounded-lg p-6">
                  <div className="space-y-3 text-left">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-medium">{seats[0].section}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-gray-600">등급</span>
                      <span className="font-medium">{seats[0].seatLevel}</span>
                    </div>
            ...
            {availableSeatsCount > 0 ? (
              <>
                <button 
                  onClick={handleFreeOrStandingBooking} 
                  disabled={processing} 
                  className="btn-primary text-lg px-12 py-4"
                >
                  {processing ? '처리 중...' : '예매하기'}
                </button>
                <p className="text-xs text-gray-500 mt-4">예매 후 5분 내 결제하지 않으면 자동 취소됩니다</p>
              </>
            ) : (
